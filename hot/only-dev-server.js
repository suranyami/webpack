	...
	var check = function check() {
		module.hot.check().then(function(updatedModules) {
			...
			if(["abort", "fail"].indexOf(status) >= 0) {
				log.warn("[HMR] Cannot check for update. Need to do a full reload!");
				log.warn("[HMR] " + err.stack || err.message);
			} else {
				log.warn("[HMR] Update check failed: " + err.stack || err.message);
			}
		});
	};
	...
	hotEmitter.on("webpackHotUpdate", function(currentHash) {
		...
		if(!upToDate()) {
			...
			if(status === "idle") {
				...
				log.warn("[HMR] Cannot apply update as a previous update " + status + "ed. Need to do a full reload!");
			}
		}
	});
